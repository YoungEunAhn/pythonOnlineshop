$(function(){
    ...
    $('.order-form').on('submit', function(e){
        ...
        //결재정보가 만들어졌으면 iamport로 실제 결제 시도
        if(merchant_id !== ''){
            IMP.request_pay({
                merchant_uid: merchant_id,
                name: 'E-Shop product',
                buyer_name:$('input[name="email"]').val(),
                amount: amount
            },function (rsp){
            console.log("rsp::::"+rsp);
            console.log("rsp_success::::"+rsp.success);
                if (rsp.success) {
                    var msg = '결제가 완료되었습니다.';
                    ...
                    var impTransaction = ImpTransaction(e, order_id, rsp.merchant_uid, rsp.imp_uid, rsp.paid_amount);
                    alert("결제끝::::::");
                } else {
                    var msg = '결제에 실패하였습니다.';
